...
describe("printDaemonStatus", () => {
  ...
  it("reports the installed service version when the Gateway never reported one", () => {
    printDaemonStatus(
      {
        cli: {
          version: "2026.6.35",
          entrypoint: "/home/ops/.local/bin/openclaw",
        },
        service: {
          label: "Daemon",
          loadState: { status: "loaded" },
    ...
    expectMockLineContains(runtime.log, "Gateway service version: 2026.4.15");
    expectMockLineContains(runtime.log, ".npm-global/lib/node_modules/openclaw");
    expectMockLineContains(runtime.error, "the installed Gateway service is version 2026.4.15");
    expectMockLineContains(runtime.error, "gateway install");
  });
  ...
  it("does not warn about the service install when it matches the CLI version", () => {
    ...
    expect(runtime.error).not.toHaveBeenCalledWith(
      expect.stringContaining("installed Gateway service is version"),
    );
  });
